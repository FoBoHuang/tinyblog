...
// 参数：
// - ctx: 上下文对象
// - client: TinyBlogClient 客户端，用于调用登录接口
// 返回：
// - 一个附加了管理员 Token 的上下文对象
func MustWithAdminToken(ctx context.Context, client apiv1.TinyBlogClient) context.Context {
	// 使用 root 用户登录
	loginResponse, err := client.Login(ctx, &apiv1.LoginRequest{
		Username: "root",         // 固定的管理员用户名
		Password: "tinyblog1234", // 固定的管理员密码
	})
	if err != nil {
