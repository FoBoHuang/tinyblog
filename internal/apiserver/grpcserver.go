...
import (
	...
	handler "github.com/FoBoHuang/tinyblog/internal/apiserver/handler/grpc"
	mw "github.com/FoBoHuang/tinyblog/internal/pkg/middleware/grpc"
...
func (c *ServerConfig) NewGRPCServerOr() (server.Server, error) {
	...
	grpcsrv, err := server.NewGRPCServer(
		...
		func(s grpc.ServiceRegistrar) {
			apiv1.RegisterTinyBlogServer(s, handler.NewHandler(c.biz))
		},
	)
	if err != nil {
		return nil, err
	...
	httpsrv, err := server.NewGRPCGatewayServer(
		...
		func(mux *runtime.ServeMux, conn *grpc.ClientConn) error {
			return apiv1.RegisterTinyBlogHandler(context.Background(), mux, conn)
		},
	)
	if err != nil {
		return nil, err
...
func NewAuthnWhiteListMatcher() selector.Matcher {
	whitelist := map[string]struct{}{
		apiv1.TinyBlog_Healthz_FullMethodName:    {},
		apiv1.TinyBlog_CreateUser_FullMethodName: {},
		apiv1.TinyBlog_Login_FullMethodName:      {},
	}
