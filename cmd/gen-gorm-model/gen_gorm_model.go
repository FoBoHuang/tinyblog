...
var generateConfigs = map[string]GenerateConfig{
	"mb": {ModelPackagePath: "../../internal/apiserver/model", GenerateFunc: GenerateTinyBlogModels},
}

// 命令行参数.
var (
	addr       = pflag.StringP("addr", "a", "127.0.0.1:3306", "MySQL host address.")
	username   = pflag.StringP("username", "u", "tinyblog", "Username to connect to the database.")
	password   = pflag.StringP("password", "p", "tinyblog1234", "Password to use when connecting to the database.")
	database   = pflag.StringP("db", "d", "tinyblog", "Database name to connect to.")
...
// GenerateTinyBlogModels 为 tinyblog 组件生成模型.
func GenerateTinyBlogModels(g *gen.Generator) {
	g.GenerateModelAs(
		"user",
		"UserM",
